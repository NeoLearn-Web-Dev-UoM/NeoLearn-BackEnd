...
document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch(`/neolearn-backend/index.php/students/search/${user.id}/courses`)
...
function createCoursesElements(courses) {
  ...
  courses.forEach((course, index) => {
    ...
    courseElement.innerHTML = `
                    <a class="lesson-link" onclick="openNewPage('lessonText${
                      index + 1
                    }', 'sourceTitle${index + 1}')">
                        <figure>
                            <blockquote class="blockquote">
                                <p id="lessonText${index + 1}">${
      course.name
    }</p>
                            </blockquote>
                            <figcaption class="blockquote-footer">
                                <cite title="Source Title" id="sourceTitle${
                                  index + 1
                                }">${course.description}</cite>
                            </figcaption>
                        </figure>
                    </a>
                    <hr>
...
function logOutBtn() {
  ...
  if (
    confirm(
      "Είσαι σίγουρος ότι επιθυμείς να αποσυνδεθείς από το Λογαριασμό σου;"
    )
  ) {
    window.location.href = "welcome.html";
  }
